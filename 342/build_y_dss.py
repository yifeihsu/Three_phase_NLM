...
def build_global_y_per_unit():
    ...
    for line_name in dss.Lines.AllNames():
        ...
        # Node order local
        local_nodes = get_local_node_names()  # e.g. ["bus1.1", "bus1.2", ... "bus2.1", ...]
        # Convert the flat array (pairs of real,imag) to complex
        cplx_list = []
        for i in range(0, len(yprim_flat), 2):
            cplx_list.append(yprim_flat[i] + 1j * yprim_flat[i + 1])
        yprim_array = np.array(cplx_list)
        n_local = int(np.sqrt(len(cplx_list)))  # dimension
...
def main():
    ...
    Y = np.array(Y)
    # Reshape Y to a square matrix
    length = np.sqrt(len(Y))
    Y = Y.reshape((int(length), int(length)))
